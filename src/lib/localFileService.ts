...
export async function uploadFileToLocalServer(
  ...
  console.log('[localFileService] Server Action: Received request to upload file.');
  try {
    const file = clientFormData.get('file') as File | null;

    if (!file) {
      const errorMsg = "Server Action Error: No file data received from client. Ensure the 'file' key is used in FormData.";
      console.error('[localFileService] Error:', errorMsg);
      console.log('[localFileService] Server Action: Returning failure (no file).');
    ...
    console.log(`[localFileService] Attempting to upload file: "${file.name}" (size: ${file.size} bytes) to ${LOCAL_SERVER_URL}`);
    ...
    try {
      ...
      console.log(`[localFileService] Server Action: Returning failure (fetch error for ${file.name}).`);
    ...
    if (!response.ok) {
      ...
      if (responseText) {
        try {
          const errorResult = JSON.parse(responseText); // Attempt to parse, might be JSON from helper
      ...
      console.log(`[localFileService] Server Action: Returning failure (response not ok for ${file.name}).`);
    ...
    if (!responseText && response.ok) {
      ...
      console.log(`[localFileService] Server Action: Returning failure (empty/invalid successful response for ${file.name}).`);
    ...
    try {
      ...
      if (result.status === "ok" && result.path) {
        ...
        console.log(`[localFileService] Server Action: Returning success for ${file.name}.`);
        ...
        console.log(`[localFileService] Server Action: Returning failure (issue in successful response for ${file.name}).`);
      ...
      console.log(`[localFileService] Server Action: Returning failure (JSON parse error for ${file.name}).`);
      return { success: false, message: parseErrorMessage };
    }

  } catch (criticalError: any) { 
    const userMessage = `Server Action Critical Error while processing file. Details: ${ (criticalError instanceof Error ? criticalError.message : String(criticalError)).substring(0, 250) }`;
    console.error(
      `[localFileService] SERVER ACTION CRITICAL UNHANDLED ERROR. Type: ${criticalError?.constructor?.name}. Message: ${criticalError?.message || 'N/A'}. Stack: ${criticalError?.stack || 'N/A'}. Cause: ${criticalError?.cause ? JSON.stringify(criticalError.cause) : 'N/A'}. Full Error Object:`, criticalError
    );
    console.log(`[localFileService] Server Action: Returning failure (critical unhandled error).`);
    return { success: false, message: userMessage };
  }
}
