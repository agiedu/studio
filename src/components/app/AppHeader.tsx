...
import { getCurrentUser, logout, isAdminSessionActive, restoreSession } from '@/lib/authService';
...
export function AppHeader() {
  ...
  useEffect(() => {
    restoreSession().then((currentUser) => {
      setUser(currentUser);
      setIsAdmin(isAdminSessionActive());
      if (!currentUser && PROTECTED_ROUTES.some(route => pathname.startsWith(route))) router.replace('/');
    });
  }, [pathname, router]);
